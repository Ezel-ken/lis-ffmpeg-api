...
app.post('/render', async (req, res) => {
  const { image_url, audio_id, duration = 15, width = 480, height = 854 } = req.body;
  ...
  try {
    ...
    if (audio_id) {
     ...
     ffmpegCmd = `ffmpeg -y -loop 1 -i "${imagePath}" -i "${audioPath}" -vf "scale=480:854:flags=fast_bilinear,format=yuv420p" -c:v libx264 -preset ultrafast -tune stillimage -crf 35 -c:a aac -b:a 64k -t ${duration} -shortest -threads 1 -max_muxing_queue_size 400 "${outputPath}"`;
    } else {
